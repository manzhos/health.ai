...
import CommunicationByTicket from './CommunicationByTicket';
// ----------------------------------------------------------------------
...
export default function InboxClient(clientId) {
  ...
  useEffect(() => {getMessagesById()}, [getMessagesById])

  async function getClient() {
    if(!clientId.clientId) return;
    try {
      const client = await request(`${API_URL}api/user/${clientId.clientId}`, 'GET', null, {
        Authorization: `Bearer ${token}`
      })
      return client
    } catch (e) { console.log('error:', e)}
                    ...
                    {filteredMessages.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((row) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <TableCell align="left">{stat(status)}</TableCell>
                          ...
                          <TableCell align="left">
                            <CommunicationByTicket ticket={ticket} message={row} procedure={procedure} client={client} />
                          </TableCell>
                        </TableRow>
                      );
                    })}
