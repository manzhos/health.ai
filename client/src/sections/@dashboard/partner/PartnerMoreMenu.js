...
export default function PartnerMoreMenu({id, partner, roleList, onChange}) {
  // console.log('Partner:', partner)
  const {token} = useContext(AuthContext)
  const ref = useRef(null);
  const navigate = useNavigate();
  const [isOpen, setIsOpen] = useState(false);
  const [role, setRole] = useState(partner.partnertype_id)
  ...
  const handleDelete = async (event) => {
    ...
    try {
      await request(`${API_URL}api/delpartner/${id}`, 'GET', null, {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      await fetch(`${API_URL}api/partner/${id}`, {
        method: 'POST', 
        body: formData,
      })
      setOpen(false)
  ...
  const avatar = () => {
    // console.log('partner avatar:', partner.avatar ? partner.avatar : 'none');
    if(partner.avatar && partner.avatar?.slice(0,4) === 'http') return partner.avatar;
    if(partner.avatar) return URL + 'files/avatars/' + partner.avatar;
  ...
  const handleCommunicate = () => {
    ...
    navigate(`/admin/partner/communication/client/${id}`);
  }
  // const handleInvoice = () => {
                ...
                <Grid container item spacing={2} xs={12} sm={12}>
                  <Grid item xs={12} sm={12} justifyContent='center'>
                    <Box sx={{ borderTop: 1, borderBottom: 1, borderColor: 'divider', mt: 0, mb: 2 }}>
                      <Typography component="h3" variant="h5" sx={{ mt: 2, mb: 2 }}>{partner.firstname} {partner.lastname}</Typography>
                    </Box>
                  </Grid>
